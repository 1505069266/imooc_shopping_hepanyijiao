<template>
  <div>
              ...
              <li v-for="(item,index) in cartList" :key="index">
                <div class="cart-tab-1">
                  <div class="cart-item-check">
                    <a  class="checkbox-btn item-check-btn" :class="{'check':Number(item.checked)}" @click="editCart('checked',item)">
    ...
    <Modal :mdShow = "mdShow" @close="closeCartDel">
      <span slot="header">提示</span>
      <p slot="msg">你确定要删除此条数据吗?</p>
      <div slot="btnGroup">
        <button  @click="closeCartDel" class="btn btn--m">关闭</button>
        <button  @click="delCart" class="btn btn--m">确认</button>
      </div>
    </Modal>
